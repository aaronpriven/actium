...
sub START {
	...
	%i18n = %{ $actium_db->all_in_columns_key(qw(I18N en sp zh)) };
	...
	say "Action\tStopID\tMainbox\tInstruction";
	...
	for \my @stop( @{$list} ) {
		...
		my $bagtext   = $stop[ $col{'Bag / Temp Sign Text'} ];
		my $desc      = $stop[ $col{'Stop Description'} ];
		my $bagortmp  = $stop[ $col{'Bag/ Tmp'} ];

		next unless u::feq( $bagortmp, 'B' );
       ...
       #<<<
		...
		say "$action\t$stopid\t$mainbox\t$stopid $desc\n";
