...
Actium::CLI - Command line interface routines for Actium
...
Actium::CLI is an object representing everything associated with the
...
presented (Actium::O::GUI, Actium::O::Web, or whatever), although none
...
encompasses much more than Actium::CLI does.  Typically this object
is called C<$env> in modules (as in Moonpig), since there could in
theory be more than one kind of environment.

Actium::CLI is a Moose object. This may surprise people who are used
to thinking of Moose as slow and inappropriate for command-line
programs. Speed was not a significant factor in the design.

=head2 Basic structure of Actium::CLI

Actium::CLI is designed to create commands with subcommands and
...
The executable script is run, and passes Actium::CLI information on
valid subcommands and other information about the system.

=item *

Actium::CLI identifies which command is selected, if any. It
...
The Actium::CLI object is constructed using a call to
Actium::CLI->new() from the main script.
...
Then Actium::CLI would use the module C<Actium::Cmd::Frederick> if
...
The value 'help' is used by Actium::CLI itself so no subcommand can
be called 'help'.
...
passed to Actium::O::Folder), representing the user's home folder. If
...
will be passed to Actium::O::Folder). This represents the folder where
...
Actium::CLI will load those modules and invoke them if selected by
the user.
...
All these methods are passed the Actium::CLI object itself as the
only argument.
...
The OPTIONS() method is how modules request that Actium::CLI do
...
An Actium::O::Folder object representing the user's home folder; see
"home_folder" above, under "Construction".

=head3 bin

An Actium::O::Folder object representing the folder where the script is
...
provided to Actium::CLI. The command name, subcommand name, and
...
Actium::CLI is designed to handle configuration and options
...
Actium::CLI. Using it is effectively the same as using the help
subcommand.  (Note that a command shell may use ? as a wildcard
character and not pass it through to Actium::CLI.)
...
"maxdepth" to 0). See Actium::O::Crier for more information. Submodules
...
colorize output.  See Actium::O::Crier for more information. Submodules
...
Actium::CLI called 'actiumdb', which will return the Actium database
object.
...
These packages allow the specification of Actium::O::Folders::Signup
...
If these packages are chosen, Actium::CLI attributes called "signup"
...
If none of these are set, Actium::O::Folders::Signup uses the C<bin>
...
package will add an attribute "flickr_auth" to Actium::CLI, which is
an Actium::O::Photos::Flickr::Auth object representing the Flickr
authorization. See Actium::O::Photos::Flickr::Auth for more
information.
...
Actium::CLI also provides some simple user help capabilities. They
...
Actium::CLI (probably a spelling error).
