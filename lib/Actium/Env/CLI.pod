...
Actium::Env::CLI is an object representing everything associated with the
...
In theory there could also be other environments that could be presented
(Actium::O::GUI, Actium::O::Web, or whatever), although none exist at the
moment.  The basic idea comes from L<Mark Jason Dominus's article on the
Moonpig billing system|http://blog.plover.com/prog/Moonpig.html>, although that
encompasses much more than Actium::Env::CLI does.  Typically this object is
called C<$env> in modules (as in Moonpig), since there could in theory be more
than one kind of environment.

Actium::Env::CLI is a Moose object. This may surprise people who are used to
thinking of Moose as slow and inappropriate for command-line programs.  Speed
was not a significant factor in the design.

=head2 Basic structure of Actium::Env::CLI

Actium::Env::CLI is designed to create commands with subcommands and which
...
The executable script is run, and passes Actium::Env::CLI information on
valid subcommands and other information about the system.

=item *

Actium::Env::CLI identifies which command is selected, if any. It displays
...
The "new" call takes the following named parameters, which should be passed as
...
The system name is intended to be an overall name for the system. In practice,
it will probably always be "actium", but it could be anything. It is used for
determining the name of the configuration files and any system environment
variables that are used.

=head3 commandpath

The command path is used for determining the name of this command, which itself
is primarily intended for display to the user. In normal usage it should be set
to perl's C<$0> variable (aka C<$PROGRAM_NAME> under L<English.pm|English>).
...
Note that the value of the C<command> attribute will always be the non-alias
name, even if the alias is specified.

The value 'help' is used by Actium::Env::CLI itself so no subcommand can be
called 'help'.
...
This should be a hash reference.  If not specified, it will be set to a I<copy>
of the system environment (perl's C<%ENV>). As such, it should be used only for
reading the environment and not for passing data to child processes.
(Functions respecting C<sysenv> could be added to Actium::Env::CLI.)

=head3 argv

This should be an array reference. If not specified, it will be set to a copy
of the command-line arguments (perl's C<@ARGV>). Note that, once created, the
C<argv> method returns the arguments also, but with any subcommand or options
removed.
...
folder. If not specified, it will be set to C<< Actium::Storage::Folder->home >>.
...
Actium::Env::CLI will load those modules and invoke them if selected by the
user.
...
None of these methods are passed as arguments. Use the C<env> subroutine from L<the Actium module|Actium> to access the environment object.
...
The OPTIONS() method is how modules request that Actium::Env::CLI do options
...
The subcommand that was chosen on the command line. So if the script is invoked
as "myscript do_something" then subcommand will be "do_something". Note that
aliases will return the original name, not the alias.
...
This is an Actium::Env::CLI::Crier object.  If not specified, it is the default
result from Actium::Env::CLI::Crier->new().  L<See
...
Actium::Env::CLI is designed to handle configuration and options processing
...
If this is set to a true value, it will prompt the user for the value of the
option if no other source for the option is available.  If this is "1", then it
will display the description, above, as the prompt string; otherwise this
string itself is used.  A colon is added to the prompt string if not already
present.
...
Actium::Env::CLI::Crier|Actium::Env::CLI::Crier> for more information.
Submodules may also inspect this option and turn off some information display.
(Negatable)
...
Actium::Env::CLI:Crier will output control codes to the terminal that will
colorize output.  
L<See Actium::Env::CLI::Crier|Actium::Env::CLI::Crier> for more information. 
...
Actium::Env::CLI::Crier will output dynamic progress indications (from the
"prog" or "over" methods).  
L<See Actium::Env::CLI::Crier|Actium::Env::CLI::Crier> for more information. 
...
Actium::Env::CLI called 'actiumdb', which will return the Actium database
object.
...
These packages allow the specification of Actium::Signup objects. See that
module for more information.

The 'signup' package only allows for the specification of the current signup,
while 'signup_with_old' allows for the specification of the current signup as
well as a previous signup.

The 'newsignup' package acts exactly the same as the 'signup' package except
that it will create the signup folder if it does not already exist.

If these packages are chosen, Actium::Env::CLI attributes called "signup" and
"oldsignup" will return the object representing the current signup and the
object representing the previous signup, respectively.
...
Actium::Env::CLI also provides some simple user help capabilities. They can
