...
Actium::Date is an subclass of L<DateTime|DateTime>.  The differences
between it and DateTime is that it is immutable (it has
L<DateTimeX::Role::Immutable|DateTimeX::Role::Immutable> applied to it
-- see that role for more information about mutators),  and it has a
few convenience  convenience methods and convenience ways of
constructing the object.

Actium::Date was created in order to do comparisons and presentation of
dates rather than times. Therefore, its own methods ignore such details
as time zones, leap seconds and the like.  Theoretically you could use
Actium::Date objects to do processing on time, but it's not really
...
The arguments used by Actium::Date are given below. If none of these
arguments are present, the arguments are passed through to DateTime.
Only one of "datetime", "strptime" or "cldr" can be present,  and if
one is, none of the other arguments other than "pattern" can be
present.
...
Actium::Date objects, DateTime objects, or strings; if they are strings
they will be formatted as dates as though they were passed to new() in
the strptime argument.  The return value is an Actium::Date object.
