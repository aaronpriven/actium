...
method _build_boxes {
    ...
    foreach my $stop_to_import ( $self->allstopids ) {
        ...
        my $box_stopid = $self->is_simple ? $EMPTY : $stop_to_import;
        ...
        while (<$kpoint>) {
            ...
            my $linegroup = $box->linegroup;
            my $line_agency
              = $self->actiumdb->line_row_r($linegroup)->{agency_id};

            next if $line_agency ne $self->agency;

            push @found_linedirs, $linegroup;
            my $dircode = $box->dircode;
...
sub _i18n_all {
    ...
    return $self->actiumdb->i18n_all_indd($i18n_id);
  ...
  COLUMN:
    foreach my $box ( $self->boxes ) {
        ...
        foreach my $attr (@attrs) {
            foreach my $i ( 0 .. $box->time_count - 1 ) {
                $seen{$attr}{ $box->$attr($i) }++;
            }
  ...
  COLUMN:
    foreach my $column ( $self->columns ) {
        ...
        if ($column_length) {
            ...
            $formatted_columns = join( ( ( $IDBOXBREAK x 2 ) ), @ft );
...
sub format_bottom {
    ...
    print $botfh $IDBOXBREAK;    # if not simple, leave blank

    ### SMOKING BOX ####

    print $botfh $IDT->parastyle('bottomnotes'),
      $IDT->encode_high_chars_only( $self->smoking ), $IDBOXBREAK;
