...
my %compare;
open my $comp, '<', 'tclist.txt' or die $!;
while (<$comp>) {
    chomp;
...
foreach my $file (qw(baglisttc.txt )) {
...
'Want more info? ' . IDTags::softreturn . 'Visit www.actransit.org or call 511 (and say, <0x201C>AC Transit.<0x201D>)'
            );
   ...
   #    }
        ...
        if ( $texts_count > $number_in_a_group ) {
            ...
            while ( my @texts = $it->() ) {
                ...
                print "bagtexts/new/$fname-$suffix.txt\n";
                open my $bagtext, '>', "bagtexts/new/$fname-$suffix.txt" or die $!;
                print $bagtext start();
                foreach (@texts) {
            ...
            print "bagtexts/new/$fname.txt\n";
            open my $bagtext, '>', "bagtexts/new/$fname.txt" or die $!;
