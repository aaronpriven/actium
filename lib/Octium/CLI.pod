...
Octium::CLI - Command line interface routines for Actium
...
Actium/Cmd/DoSomething.pm:
 ...
 package Octium::Cmd::DoSomething 0.001;
...
Octium::CLI is an object representing everything associated with the
command-line interface for the Actium system.
...
presented (Octium::O::GUI, Octium::O::Web, or whatever), although none
exist at the moment.  The basic idea comes from L<Mark Jason Dominus's
article on the Moonpig billing
system|http://blog.plover.com/prog/Moonpig.html>, although that
encompasses much more than Octium::CLI does.  Typically this object
is called C<$env> in modules (as in Moonpig), since there could in
theory be more than one kind of environment.

Octium::CLI is a Moose object. This may surprise people who are used
to thinking of Moose as slow and inappropriate for command-line
programs. Speed was not a significant factor in the design.

=head2 Basic structure of Octium::CLI

Octium::CLI is designed to create commands with subcommands and
...
The executable script is run, and passes Octium::CLI information on
valid subcommands and other information about the system.

=item *

Octium::CLI identifies which command is selected, if any. It
...
to display its help information.

=back

=head1 CONSTRUCTION

The Octium::CLI object is constructed using a call to
Octium::CLI->new() from the main script.

The "new" call takes the following named parameters, which can be
passed as a hash reference or as a plain hash (in other words, a simple
...
Then Octium::CLI would use the module C<Octium::Cmd::Frederick> if
either 'fred' or 'freddy' were supplied, and use the module
C<Octium::Cmd::Sam::Spade> if 'sam_spade' were supplied.

Note that the value of the C<command> attribute will always be the
non-alias name, even if the alias is specified.

The value 'help' is used by Octium::CLI itself so no subcommand can
be called 'help'.
...
passed to Octium::O::Folder), representing the user's home folder. If
not specified, it will be set to the user's home folder (as determined
by L<the File::HomeDir module|File::HomeDir>.

=head3 bin

This should also be a string representing a path on disk (which, again,
will be passed to Octium::O::Folder). This represents the folder where
the script is stored. If not specified, it will be set to C<$Bin> from
L<the FindBin module|FindBin>.

=head1 SUBCOMMAND MODULES

Subcommand modules will be found in the "Octium::Cmd" namespace.
Octium::CLI will load those modules and invoke them if selected by
the user.
...
All these methods are passed the Octium::CLI object itself as the
only argument.
...
The OPTIONS() method is how modules request that Octium::CLI do
...
An Octium::O::Folder object representing the user's home folder; see
"home_folder" above, under "Construction".

=head3 bin

An Octium::O::Folder object representing the folder where the script is
located; see "bin" above, under "Construction".

=head3 config

An Octium::Storage::Ini object representing a configuration file. The
...
file will be ".actium.ini"). See Octium::Storage::Ini for details.
...
"Octium::Cmd::DoSomething".

=head3 crier

This is an Octium::O::Crier object.  At the moment it will be the
result from default_crier(). See "Octium::O::Crier" for more
information.
...
provided to Octium::CLI. The command name, subcommand name, and
...
Octium::CLI is designed to handle configuration and options
...
displaying this string, if there is no other source for the value. A
colon is added to the prompt string if not already present.
...
Octium::CLI. Using it is effectively the same as using the help
subcommand.  (Note that a command shell may use ? as a wildcard
character and not pass it through to Octium::CLI.)
...
This will stop Octium::O::Crier from outputting most text (it sets
"maxdepth" to 0). See Octium::O::Crier for more information. Submodules
may also inspect this option and turn off some information display.
(Negatable)
...
Octium::O::Crier will output control codes to the terminal that will
colorize output.  See Octium::O::Crier for more information. Submodules
may also inspect this option and turn on color display. (Negatable)
...
Octium::O::Crier will output dynamic progress indications (from the
"prog" or "over" methods).  See Octium::O::Crier for more information.
...
See Octium::O::Files::ActiumDB for more information.

When this package is requested, it will create a new attribute in
Octium::CLI called 'actiumdb', which will return the Actium database
object.
...
These packages allow the specification of Octium::O::Folders::Signup
objects. See that module for more information.
...
If these packages are chosen, Octium::CLI attributes called "signup"
and "oldsignup" will return the object representing the current signup
and the object representing the previous signup, respectively.
...
If none of these are set, Octium::O::Folders::Signup uses the C<bin>
attribute to find the script's folder, and sets the base folder to
"signups" in the script folder's parent folder.  In other words, it's
something like "/Actium/bin/../signups". In the normal case where the
"bin" folder is in the same folder as the Actium data this means it
will all work fine without any specification of the base folder. If
that is not the case, an error will be generated.
...
A folder where local cache files are stored.  The fallback value is a
directory called '.' plus the system name (so, if the system name is
...
L<Octium::Geo|Octium::Geo> for more information.
...
=head2 'flickr' package

This package is used for accessing the Flickr api.  If selected, this
package will add an attribute "flickr_auth" to Octium::CLI, which is
an Octium::O::Photos::Flickr::Auth object representing the Flickr
authorization. See Octium::O::Photos::Flickr::Auth for more
information.

=head3 option: flickr_key

=head3 environment: FLICKR_KEY

=head3 config key: 'key' in section 'Flickr'

The "key" required to use the Flickr API. If not provided, the user
will be prompted for the key.

=head3 option: flickr_secret

=head3 environment: FLICKR_SECRET

=head3 config key: 'secret' in section 'Flickr'

The "secret" required to use the Flickr API. If not provided, the user
will be prompted for the key.

=head1 USER HELP

Octium::CLI also provides some simple user help capabilities. They
...
Octium::CLI (probably a spelling error).
