...
print '<h2>Other Retail Locations</h2><ul style="-webkit-column-count: 3; -webkit-column-gap: 10px; -moz-column-count: 3; -moz-column-gap: 10px; column-count:3; column-gap:10px;list-style-type:square; ">';
...
sub stores_table {
   ...
   foreach my $store_r (@stores) {
      ...
      my $mapurl = 'http://maps.google.com/maps?q=@' . "$fields[1],%20$fields[2]%20CA&z=18";

      $fields[0] .= "*" if $store_r->{notes} =~ /BART Ticket/;

      $fields[1] = qq{<a href="$mapurl" target=_blank >} . $fields[1] . q{</a>};
