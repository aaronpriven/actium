...
is located; see "bin" above, under "Construction".
...
"system_name" above, under "Construction".

=head3 module

The package name of the module that is associated with this subcommand.
If the "do_something" subcommand is associated with "DoSomething" in the
...
L<"Configuring Getopt::Long" in Getopt::Long|Getopt::Long/"Configuring Getopt::Long">. A single hyphen is used to introduce all options.
...
If these packages are chosen, Actium::O::Cmd attributes called
...
This package is used for geocoding using the geonames.org server.
See L<Actium::Geo|Actium::Geo> for more information.
...
package will add an attribute "flickr_auth" to Actium::O::Cmd, which
is an Actium::O::Photos::Flickr::Auth object representing the Flickr
...
The "key" required to use the Flickr API.
If not provided, the user will be prompted for the key.
...
The "secret" required to use the Flickr API.
If not provided, the user will be prompted for the key.

=head1 USER HELP

Actium::O::Cmd also provides some simple user help capabilities.
They can be invoked in two ways: via the "help" subcommand, or the
"-help" (or "-?") option.
...
options is displayed, using the "description" from the
option specification.  If "display_default" was specified, then
the default will be displayed in the option summary. (Note that
this default is whatever will be used if no option is specified on
the command line. It will display a value found in an environment
variable or configuration file, for example.)

All option names are supplied, even aliases, although the description
of every alias is just "Same as" followed by the name of the main
name.
...
method was found in the module.  Command-line option usage will
be displayed anyway.
...
Actium::Moose and its dependencies
