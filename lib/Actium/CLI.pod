...
encompasses much more than Actium::CLI does.  Typically this object is
called C<$env> in modules (as in Moonpig), since there could in theory
be more than one kind of environment.

Actium::CLI is a Moose object. This may surprise people who are used to
...
Actium::CLI is designed to create commands with subcommands and which
...
Actium::CLI identifies which command is selected, if any. It displays
...
The value 'help' is used by Actium::CLI itself so no subcommand can be
called 'help'.
...
folder. If not specified, it will be set to the user's home folder (as
determined by L<the File::HomeDir module|File::HomeDir>.
...
Actium::CLI will load those modules and invoke them if selected by the
user.
...
All these methods are passed the Actium::CLI object itself as the only
argument.
...
The OPTIONS() method is how modules request that Actium::CLI do options
...
Actium::CLI is designed to handle configuration and options processing
...
The 'newsignup' package acts exactly the same as the 'signup' page
...
package will add an attribute "flickr_auth" to Actium::CLI, which is an
Actium::O::Photos::Flickr::Auth object representing the Flickr
...
Actium::CLI also provides some simple user help capabilities. They can
