...
# The preamble to Octium perl modules.
...
Octium - Common routines and imports for Octium programs
...
=head1 SYNOPSIS
 ...
 use Octium; # in a procedural module or non-Moose class

 use Octium ('class'); # in a Moose class

 use Octium ('role'); # in a Moose role

=head1 DESCRIPTION

Octium.pm provides the boilerplate code that should be common to all
Octium modules. It contains a number of utility routines, exports a
...
This should be moved to Octium::O::Dir when other modules not using it
are changed.
...
the Lines table in the Octium database used instead.
...
into modules that use Octium with the 'class' parameter.
...
In addition to modules listed later, the following module is imported
into modules that use Octium with the 'role' parameter.
...
In addition to modules listed later, the following module is imported
into modules that use Octium with either the 'class' or the 'role'
parameter.
...

{
    ...
    sub import {
        ...
        _do_import 'Octium::Crier', qw/cry last_cry/;
        _do_import 'Carp';
...
qualified name, e.g. "Octium::byline".

The only subroutine that is exported is C<env>. It is always exported.

As a convenience, the package "u" was made an alias for the "Octium"
...
not that much harder to type "Octium::byline", especially if "Octium::"
...
At the moment this is always going to be an Octium::CLI object,
...
L<C<byline> from Octium::Sorting::Line|Octium::Sorting::Line/byline>
...
L<C<sortbyline> from
Octium::Sorting::Line|Octium::Sorting::Line/sortbyline>
...
No arguments passed to Octium::joinseries

The joinseries function was called without any valid arguments. Supply
strings to join.

Odd number of elements passed to Octium::hashref
...
Octium.pm requires the following modules or distributions to be
present:
