...
sub build_point {
   ...
   $point->{HEADNUM} = [sort byline @{$point->{ROUTES2USE}}];
...
sub headdays {
   ...
   # if there's only one day present,
      ...
      if ($used[0] eq "BLANK") {
         ...
         $daystring = $Skedvars::shortdaynames{$daycode};
...
#   }
   ...
   mkdir "tapoints" or die 'Can\'t make directory "tapoints"'  
                  unless -d "tapoints";

#   open OUT, ">tapoints/$signid-" . $signs{$signid}{SignType} . "-" . 
#                  scalar (@points) . ".txt";

   open OUT, ">tapoints/$signid.txt";

   print OUT IDTags::start_with_tables;
...
#        return -1 if $b->{"HEADNUM"}[0] eq "82L" and $a->{"HEADNUM"}[0] eq "801";
   ...
   foreach my $point (@points) {
      ...
      # Numbers (without names), and days

      my @headnums = @{$point->{"HEADNUM"}};
      my $lengthheadnums = length(join("/" , @headnums));
      
      my ($tablenum, $tabledays, $tabledest) = ('', '', '');

      if ($#headnums == 0) { # if there's only one head number 
                            # (i.e., last one is #0),
                            
         $tabledays = IDTags::parastyle('days') . $point->{HEADDAYS};

         $tablenum = IDTags::parastyle("linenumber");
         $tablenum .= IDTags::color(getcolor($headnums[0]) , $headnums[0]);

      } else { # multiple head numbers....

         $tabledays = IDTags::parastyle('dayslines') . $point->{HEADDAYS};
         $tablenum = IDTags::parastyle("linesnumbers");
         ...
         foreach (0 .. $#headnums) {
            unless ($_ == 0) {
               ...
               $tablenum .= IDTags::color($sepcolor, "/");
            }
            $tablenum .= IDTags::color($headcolors[$_],$headnums[$_]);
         }
      }
  
      # end of numbers and days
      
      # beginning of destinations

      unless ($point->{'NO'} eq "l") { # unless it's a "last stop" note,

         $tabledest = IDTags::parastyle('ToLeave') . IDTags::bold("To ");

         # add destination
         if ($point->{DestinationNote} ) {
           $tabledest .=  $point->{HEADDEST} . " " . notrailingperiod ($point->{DestinationNote}) . ".";
         } elsif ($point->{DIR} eq "CW") {
            $tabledest .= $point->{HEADDEST} . " (Clockwise loop).";
         } elsif ($point->{DIR} eq "CC") {
            $tabledest .=  $point->{HEADDEST} . " (Counterclockwise loop).";
         } else {
            $tabledest .=  notrailingperiod($point->{HEADDEST}) . ".";
         }
         ...
         if ($point->{AddMins} == 0 and (not $noteonly) ) {
            ...
            $tabledest .= IDTags::bold(' Leaves ') . "$headtp at:"
...
#      '<TableStart:1,2:0:0<tCellDefaultCellType:Text><tOuterLeftStrokeType:None><tOuterRightStrokeType:None><tOuterTopStrokeType:None><tOuterBottomStrokeType:None><tBeforeSpace:0><tAfterSpace:0>>'
#      . '<RowStart:<tRowAutoGrow:1>>'
#      . '<CellStart:1,1>'
#      . $tablenum
#      . "\r"
#      . $tabledays
...
#      
   ...
   IDTags::parastyle("amtimes") .   '<TableStart:1,2:0:0<tCellDefaultCellType:Text><tOuterLeftStrokeType:None><tOuterRightStrokeType:None><tOuterTopStrokeType:None><tOuterBottomStrokeType:None><tBeforeSpace:0><tAfterSpace:0>><ColStart:<tColAttrWidth:62.95182799413773>><ColStart:<tColAttrWidth:145.84817200586224>><RowStart:<tRowAttrHeight:38.98799890472487><tRowAttrMinRowSize:38.98799890472487><tRowAutoGrow:0>><CellStart:1,1<tCellAttrLeftInset:0><tCellAttrTopInset:0><tCellAttrRightInset:0><tCellAttrBottomInset:0><tCellAttrLeftStrokeWeight:0><tCellAttrRightStrokeWeight:0><tCellAttrTopStrokeWeight:0><tCellAttrBottomStrokeWeight:0><tCellLeftStrokeColor:Black><tCellTopStrokeColor:Black><tCellRightStrokeColor:Black><tCellBottomStrokeColor:Black><tcLeftStrokeType:None><tcRightStrokeType:Solid><tcTopStrokeType:None><tcBottomStrokeType:None><tCellAttrLeftStrokeTint:100><tCellAttrRightStrokeTint:100><tCellAttrTopStrokeTint:100><tCellAttrBottomStrokeTint:100><tCellLeftStrokeOverprint:0><tCellRightStrokeOverprint:0><tCellTopStrokeOverprint:0><tCellBottomStrokeOverprint:0><tCellLeftStrokeGapTint:100><tCellRightStrokeGapTint:100><tCellTopStrokeGapTint:100><tCellBottomStrokeGapTint:100><tCellLeftStrokeGapColor:None><tCellRightStrokeGapColor:None><tCellTopStrokeGapColor:None><tCellBottomStrokeGapColor:None><tCellLeftStrokeGapOverprint:0><tCellRightStrokeGapOverprint:0><tCellTopStrokeGapOverprint:0><tCellBottomStrokeGapOverprint:0>>' . "$tablenum\r$tabledays" . '<CellEnd:><CellStart:1,1<tCellAttrLeftInset:0><tCellAttrTopInset:0><tCellAttrRightInset:0><tCellAttrBottomInset:0><tCellAttrLeftStrokeWeight:0><tCellLeftStrokeColor:Black><tcLeftStrokeType:Solid><tCellAttrLeftStrokeTint:100><tCellLeftStrokeOverprint:0><tCellLeftStrokeGapTint:100><tCellLeftStrokeGapColor:None><tCellLeftStrokeGapOverprint:0>>' . $tabledest . '<CellEnd:><RowEnd:><TableEnd:>';
...
=cut
         ...
         # 1R5day note
            ...
            $pointtext .= 'Buses arrive about every 12 minutes �' .
                      IDTags::softreturn . 
                      'See information elsewhere on this sign.';
            # that is an emdash - it should carry through. It's
            # potentially a crossplatform issue though.
...
sub signdescription {
   ...
   if ($uniqueid) {
      ...
      $description .= " (#$signid; stop #$uniqueid";
