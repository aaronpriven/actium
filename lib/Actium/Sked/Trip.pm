...
method calendar_noteletter {
    if ( $self->has_calendar ) {
        return $self->calendar->noteletter;
    }
    else {
        return $EMPTY;
    }
...
method calendar_note {
    if ( $self->has_calendar ) {
        return $self->calendar->note;
    }
    else {
        return $EMPTY;
    }
...
Need to redo merge_pair. Make a new trait -- get_merged_value or something --
and use a routine associated with that trait to decide what to do 
when those traits are merged. 

=cut

method merge_pair ($secondtrip) {
