...
Actium::O::Cmd is an object representing everything associated with the
...
encompasses much more than Actium::O::Cmd does.  Typically this object
is called C<$env> in modules (as in Moonpig), since there could in
theory be more than one kind of environment.

Actium::O::Cmd is a Moose object. This may surprise people who are used
...
Actium::O::Cmd is designed to create commands with subcommands and
...
Actium::O::Cmd identifies which command is selected, if any. It
...
Then Actium::O::Cmd would use the module C<Actium::Cmd::Frederick> if
...
The value 'help' is used by Actium::O::Cmd itself so no subcommand can
be called 'help'.
...
The OPTIONS() method is how modules request that Actium::O::Cmd do
...
An Actium::O::Files::Ini object representing a configuration file. The
...
file will be ".actium.ini"). See Actium::O::Files::Ini for details.
...
provided to Actium::O::Cmd. The command name, subcommand name, and
...
Actium::O::Cmd called 'actiumdb', which will return the Actium database
object.
...
If these packages are chosen, Actium::O::Cmd attributes called "signup"
...
package will add an attribute "flickr_auth" to Actium::O::Cmd, which is
...
Actium::O::Cmd also provides some simple user help capabilities. They
...
Actium::O::Cmd (probably a spelling error).
