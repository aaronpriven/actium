...
our ( @signs, @lines, @signtypes, @skedspec, @timepoints );
our ( %signs, %stops, %lines, %signtypes, %skedspec, %timepoints );
...
load_tables(
    requests => {
        ...
        Signs => { array => \@signs, hash => \%signs, index_field => 'SignID',
           fields => [qw[
           SignID Active UseOldMakepoints SignType UNIQUEID Sidenote Note600s 
           DescripNotes ShelterNum NonStopLocation NonStopCity
           ]], 
...
foreach my $signid ( sort { $a <=> $b } @signstodo ) {
    ...
    next SIGN
      unless lc( $signs{$signid}{Active} ) ne "no"
...
} ## tidy end: foreach my $signid ( sort {...})
...
sub build_used {
  ...
  ROW:
    for ( my $row = 0 ; $row < scalar @{ $point->{"ROUTES"} } ; $row++ ) {
        ...
        if ($dosecond) {
            next ROW
              unless $point->{TIMES}[$tpnum][$row]
              or $point->{TIMES}[$s_tpnum][$row];
        ...
        my $lasttpnum;
      TPS:
        for ( my $i = ( ( scalar @{ $point->{TP} } ) - 1 ) ; $i >= 0 ; $i-- ) {
        ...
        $point->{LASTTP}[$row] = $point->{TP}[$lasttpnum];
...
sub headdest {
    ...
    if ( $point->{NO} eq "l" ) {
        ...
        for ( my $row = 0 ; $row < scalar @{ $point->{ROUTES} } ; $row++ ) {
            next
              unless usedrow( $point, $row )
              and $point->{ROUTES}[$row] eq $point->{MAINROUTE};
    ...
    } ## tidy end: else [ if ( $point->{NO} eq "l")]
...
sub output_points {
    ...
    my ($head,      $thismark, @thesemarks, $route,
        $lasttp,    $temp,     $ampm,       @markdefs,
        %usedmarks, $lengthinlines
    );
 ...
 #   }
    ...
    mkdir $IDPOINTFOLDER
      or die "Can't make directory '$IDPOINTFOLDER'"
    ...
    @points = sort {
        return 1
          if $a->{"HEADNUM"}[0] eq "82" and $b->{"HEADNUM"}[0] eq "82L";
        ...
        return -1
          if $b->{"HEADNUM"}[0] eq "82L" and $a->{"HEADNUM"}[0] eq "801";
# reverse 801, 82L, 82 since usually this is what we have to end up doing by hand
        byline( $a->{"HEADNUM"}[0], $b->{"HEADNUM"}[0] )
          or $Skedvars::dirhash{ $b->{"DIR"} }
          <=> $Skedvars::dirhash{ $a->{"DIR"} }
          or $Skedvars::dayhash{ $b->{"DAY"} }
          <=> $Skedvars::dayhash{ $a->{"DAY"} }
    } @points;
    ...
    foreach my $point (@points) {
        ...
        if ( $#headnums == 0 ) {    # if there's only one head number
            ...
            $pointtext .= IDTags::parastyle("dropcaphead")
              . IDTags::dropcapchars( $lengthheadnums + 1 );
            $pointtext
              .= IDTags::color( getcolor( $headnums[0] ), $headnums[0] . " " );

        }
        else {                      # multiple head numbers....

            $pointtext .= IDTags::parastyle("dropcapheadmany")
              . IDTags::dropcapchars( $lengthheadnums + 1 );
        ...
        unless ( $point - {'NO'} eq "l" ) {    # unless it's a "last stop" note,
            ...
            if ( $point->{DestinationNote} ) {
                ...
                $pointtext
                  .= " to " . $point->{HEADDEST} . " (Clockwise loop).";
            }
                ...
                $pointtext
                  .= " to " . $point->{HEADDEST} . " (Counterclockwise loop).";
            }
            else {
                my $headdest = $point->{HEADDEST};
                $headdest =~ s/\.$//;
                $pointtext
                  .= " to " . notrailingperiod( $point->{HEADDEST} ) . ".";
            }
...
=cut
        ...
        $pointtext
          .= IDTags::superscript( join( ",", sort { $a <=> $b } @thesemarks ) )
...
=cut
                ...
                $pointtext
                  .= 'Buses arrive about every 12 minutes weekdays, and 15 minutes weekends. (Weekend service to downtown Oakland only.) '
                  . IDTags::softreturn
                  . 'See information elsewhere on this sign.';
        ...
        for ( my $row = 0 ; $row < scalar @{ $point->{ROUTES} } ; $row++ ) {
            ...
            if ( $lengthinlines and $usedcount > $lengthinlines ) {
                ...
                $pointtext
                  .= IDTags::parastyle( 'amtimes', IDTags::boxbreak() );
                $columncount++;
                $prev      = "z";
                $usedcount = 1;
            }
            ...
            } ## tidy end: if ( $point->{AddMins}...)

            if ( $ampm ne $prev ) {
                $pointtext
                  .= IDTags::parastyle( $ampm eq 'a' ? 'amtimes' : 'pmtimes' );
            ...
            if ( $route or $lasttp or $lesser ) {
                ...
                if ( $usedmarks{$_} ) {
                    ...
                    if ( $route or $lasttp ) {
                        if ($route) {
                            $temp = "Line $route";
                            $temp .= ", to " . tphash($hashlasttp) if $lasttp;
                ...
                } ## tidy end: else [ if ( $usedmarks{$_} ) ]

                push @thesemarks, $thismark;

            } ## tidy end: if ( $route or $lasttp...)
            ...
            if ( scalar(@thesemarks) ) {
                ...
                $time = "\t$time"
                  if ( ( length($time) + length($footnotes) ) < 8 );
    ...
    my $phoneid = $stops{$uniqueid}{h_stp_511_id};
    ...
    print OUT IDTags::color(
        "Rapid Red",
"Times shown in red are only approximations. Buses may come somewhat earlier or later than the time shown.\r"
    ) if $addminsflag;
...
=cut
    ...
    if ( $schooldayflag or $usedmarks{SD} ) {
        ...
        print OUT
"Trips that run school days only may not operate every day and will occasionally operate at times other than those shown. Supplementary service is available to everyone at regular fares.\r";
...
"See something wrong with this sign, or any other AC Transit sign? Let us know! Leave a comment at actransit.org/feedback or call 511 and say 'AC Transit'. Thanks!\r"
    ...
    print OUT "[$signs{$signid}{DescripNotes}] "
      if $signs{$signid}{DescripNotes};
...
sub get_head_timepoints {
    ...
    for ( my $column = ($#points) ; $column >= 0 ; $column-- ) {
    ...
    my $thistp = (
        sort { $tpfreq{$b} <=> $tpfreq{$a} or $tpcols{$a} <=> $tpcols{$b} }
          keys %tpfreq
    )[0];
...
} ## tidy end: sub get_head_timepoints
...
sub signdescription {
    ...
    if ($uniqueid) {
        ...
        my ( $on, $at ) = ( $thisstop->{c_on}, $thisstop->{c_at} );
        ...
        my $phoneid = $thisstop->{h_stp_511_id};

        my $description = "";

        my $direction = $thisstop->{c_direction};
        $direction = $Skedvars::longdirnames{$direction}
          if $Skedvars::longdirnames{$direction};

        $description .= "$thisstop->{c_street_num} "
          if $thisstop->{c_street_num};
        $description .= $on;
        $description .= " at $at" if $at;
        $description .= ", " . $thisstop->{c_city};
    ...
    } ## tidy end: if ($uniqueid)

    # if no uniqueid,

    return
        $signs{$signid}{NonStopLocation} . ", "
      . $signs{$signid}{NonStopCity}
      . ". (#$signid)";

} ## tidy end: sub signdescription
