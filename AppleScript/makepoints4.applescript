-- set override_library to load script file "Macintosh HD:Users:Shared:Dropbox (AC_PubInfSys):Actium:Applications:override_library.scpt"
set HomePath to ((path to home folder) as string)
set OverrideLibraryFile to HomePath & "Alameda - Contra Costa Transit:PubInfSys - Documents:Actium:Applications:override_library.scpt"
set override_library to load script file OverrideLibraryFile
-- there seems to be no obvious way to use a POSIX path instead of a Macintosh path
...
set MapsNotFoundFile to ListPath & ":MapsNotFound-" & ListFileName
...
set OriginalIDFileFolder to (POSIX file "/Users/apriven/Alameda - Contra Costa Transit/PubInfSys - Documents/AtStopSchedules/Templates/") as text
set NewIDPath to SignupPath & ":signart"
my EnsureFolderExists(NewIDPath)
set MapPath to HomePath & "Alameda - Contra Costa Transit:PubInfSys - Documents:Actium:signart2016:maps:export:"
	...
	try -- only used to show InDesign document window in the event of an error
		...
		repeat with theLineItem in (ListLines)
			...
			if ((item 1 of theLineFields) = "FILE") then
				...
				set MapID to third item of theLineFields
				...
				tell override_library
					...
					local myLabel
					local myMasterItems
					set myLabel to "PageName"
					set myMasterItems to (every item of all page items of applied master of thePage where label is myLabel)
					
					if ((count of myMasterItems) = 0) then
						set myLabel to "NonPrintingSignID"
						set myMasterItems to (every item of all page items of applied master of thePage where label is myLabel)
					end if
					
					
					if ((count of myMasterItems) ≠ 0) then
						overridetext(myLabel, thePage, SignID)
					end if
					
					local succeeded
					set succeeded to override_graphic_if_exists("MapFrame", thePage, (MapPath & SignType & ":" & MapID & ".pdf"))
					if not succeeded then
						copy SignID to end of MapsNotFound
					end if
				end tell
...
if (length of MapsNotFound) ≠ 0 then
	
	set text item delimiters to ", "
	
	set MapsNotFoundString to (MapsNotFound as string)
	
	set MapsFH to open for access (MapsNotFoundFile) with write permission
	write "Maps not Found:" & return to MapsFH
	write MapsNotFoundString & return to MapsFH
	close access MapsFH
	
	
	display alert "Finished making point schedules" & return & "Maps not found: " & (MapsNotFound as string) giving up after 30
